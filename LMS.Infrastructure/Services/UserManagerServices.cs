...
namespace LMS.Infrastructure.Services
{
    public class UserManagerServices : IUserManagerServices
    {
        ...
        public async Task ForgotPassword(ForgotPasswordViewModel forgotPasswordViewModel)
        {
            try
            {

                var users = await _unitOfWork.GetRepository<Employee>().GetAll();
        ...
        public async Task Send(Employee user)
        {
            try
            { 
                var emailVerficationToken = await _authService.GenerateToken(user);
                ...
                var resetPasswordVerification = await _unitOfWork.GetRepository<ResetPasswordVerification>().Add(model);
                await _unitOfWork.Save();
                var body = SendVerificationEmail(user, token);

                await _mailServices.SendEmailAsync(user.Email,"Reset Password Link", body);
